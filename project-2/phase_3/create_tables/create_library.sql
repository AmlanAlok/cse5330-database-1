...
BEGIN
	...
	if new.staff_position_id = (select sp.id from staff_position as sp where sp.staff_position = 'professor') then
		insert into `axa5861`.`member` (`member_name`,`ssn`,`campus_address`,`home_address`,`phone_no`,`card_issue_date`,`card_expiry_date`,`borrowed_count`,`member_type_id`,`member_status_id`,`renewed`)
